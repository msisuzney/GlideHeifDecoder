 ...
 * HeifBitmapFactory}.
 */
public final class HeifDownsampler {
    static final String TAG = "HeifDownsampler";
    ...
    private static final com.bumptech.glide.load.resource.bitmap.Downsampler.DecodeCallbacks EMPTY_CALLBACKS =
            new com.bumptech.glide.load.resource.bitmap.Downsampler.DecodeCallbacks() {
                @Override
     ...
     *
     * @see #decode(InputStream, int, int, Options, com.bumptech.glide.load.resource.bitmap.Downsampler.DecodeCallbacks)
     */
    ...
    public Resource<Bitmap> decode(
            ...
            throws IOException {
        return decode(
                new ImageReader.InputStreamImageReader(is, byteArrayPool),
                requestedWidth,
                requestedHeight,
                options,
                callbacks);
    ...
    public Resource<Bitmap> decode(
            ...
            throws IOException {
        return decode(
                new ImageReader.ParcelFileDescriptorImageReader(parcelFileDescriptor, byteArrayPool),
                outWidth,
                outHeight,
                options,
                EMPTY_CALLBACKS);
...
//        uint32_t bitmapHeight = imageRect.height / sampleSize;
        int bitmapWidth = sourceWidth / options.inSampleSize;
        int bitmapHeight = sourceHeight / options.inSampleSize;
        options.inBitmap = bitmapPool.get(bitmapWidth,bitmapHeight,Config.ARGB_8888);
    ...
    private static void calculateScaling(
        ...
        if (orientedSourceWidth % powerOfTwoSampleSize != 0
                || orientedSourceHeight % powerOfTwoSampleSize != 0) {
            powerOfTwoWidth = sourceWidth;
            powerOfTwoHeight = sourceHeight;
        } else {
